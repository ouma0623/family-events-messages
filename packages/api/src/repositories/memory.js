...
exports.MemoryEventRepository = void 0;
/**
 * メモリ実装（開発・テスト用）
 */
class MemoryEventRepository {
    ...
    async search(query) {
        ...
        if (query.keyword) {
            ...
            results = results.filter((e) => e.title.toLowerCase().includes(keyword) ||
     ...
     * 週末おすすめイベントを取得
     */
    async listWeekendRecommendations(userPref) {
        const now = new Date();
...
exports.MemoryEventRepository = MemoryEventRepository;
/**
 * 次の週末を取得
 */
