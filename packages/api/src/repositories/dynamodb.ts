...
export class DynamoDBEventRepository implements EventRepository {
  ...
  private summariesTableName: string;
   ...
   * 週末おすすめイベントを取得
   */
  async listWeekendRecommendations(userPref: UserPreference, weekendOf?: string): Promise<EventNormalized[]> {
    // 週末日付範囲を計算
   ...
   * 市で検索
   */
  private async searchByCity(query: SearchQuery): Promise<EventNormalized[]> {
    const params: any = {
      TableName: this.eventsTableName,
   ...
   * 都道府県で検索
   */
  private async searchByPref(query: SearchQuery): Promise<EventNormalized[]> {
    ...
    if (prefCode && prefCode !== normalizedPref) {
      ...
      const codeResult = await this.client.send(new QueryCommand(codeParams));
   ...
   * カテゴリで検索
   */
  private async searchByCategory(query: SearchQuery): Promise<EventNormalized[]> {
    if (!query.categories || query.categories.length === 0) {
      return [];
    }
   ...
   * 日付範囲で検索（全件スキャン）
   */
  private async searchByDateRange(query: SearchQuery): Promise<EventNormalized[]> {
    const params: any = {
      TableName: this.eventsTableName,
   ...
   */
  private async searchRecent(query: SearchQuery): Promise<EventNormalized[]> {
    // JST（UTC+9）で現在日時を取得
    const now = new Date();
   ...
   */
  private sortEventsByDisplayOrder(events: EventNormalized[]): EventNormalized[] {
    return events.sort((a, b) => {
      ...
      // 優先度1: イベント期間が3日以内を最優先
      if (aIsShort && !bIsShort) return -1; // aを優先
      if (!aIsShort && bIsShort) return 1;  // bを優先
   ...
   */
  private normalizeEvent(event: EventNormalized): any {
   ...
   */
  private denormalizeEvent(item: any): EventNormalized {
    const { category, ttl, ...event } = item;
    return event as EventNormalized;
  }

  /**
   * TTL計算（イベント終了日から30日後）
   */
  private calculateTTL(endAt: string): number {
  ...
  async updateUser(
    userId: string,
    preferences: Partial<UserPreference> & { lineUserId?: string; notifyEnabled?: boolean }
  ): Promise<void> {
    // 既存のユーザーデータを取得
   ...
   * 通知有効なユーザー一覧を取得
   */
  async getNotifyEnabledUsers(): Promise<any[]> {
    ...
    const allUsers = await this.client.send(
      new ScanCommand({
        TableName: this.usersTableName,
