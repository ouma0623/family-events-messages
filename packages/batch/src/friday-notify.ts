 ...
 * 週末イベントを取得
 */
function getWeekendDateRange(): { startDate: string; endDate: string } {
 ...
 * イベントをランキング
 */
function rankEvents(events: EventNormalized[], userPref: UserPreference): EventNormalized[] {
  // recommendScoreでソート
 ...
 * イベントをメッセージに整形
 */
function formatEventMessage(event: EventNormalized): string {
 ...
 * 複数イベントをメッセージに整形
 */
function formatEventsMessage(events: EventNormalized[]): string {
  if (events.length === 0) {
 ...
 * 金曜通知バッチを実行
 */
export async function runFridayNotify(
  ...
  for (const user of users) {
    ...
    try {
      ...
      errorHandler.error(`Failed to send notification to ${user.userId}`, {
        error: errorMessage,
      });
    }
