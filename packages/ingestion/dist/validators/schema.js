...
function validateSchema(event) {
    ...
    // エラーがある場合はログ出力
    if (errors.length > 0) {
        handler_1.errorHandler.error(`Validation failed for event ${event.eventId}`, {
    ...
    if (warnings.length > 0) {
        handler_1.errorHandler.warn(`Validation warnings for event ${event.eventId}`, {
            sourceId: event.sourceId,
            eventId: event.eventId,
            error: warnings.join('; '),
        });
