 ...
 * 会場名を正規化（空白/記号除去）
 */
function normalizeVenueName(venueName: string | undefined): string {
  if (!venueName) return '';
...
function calculateTitleSimilarity(title1: string, title2: string): number {
  ...
  normalized1.split(/\s+/).forEach((token) => {
    if (token.length > 0) tokens1.add(token);
  });
  normalized2.split(/\s+/).forEach((token) => {
    if (token.length > 0) tokens2.add(token);
  });
 ...
 * 日付が一致するかチェック
 */
function datesMatch(startAt1: string, endAt1: string, startAt2: string, endAt2: string): boolean {
 ...
 * 会場名が一致するかチェック
 */
function venueMatches(venue1: string | undefined, venue2: string | undefined): boolean {
 ...
 * 09_DEDUP_RULES.md セクション3 に準拠
 */
export function isSameEvent(event1: EventNormalized, event2: EventNormalized): boolean {
  // 1. 日付が一致
