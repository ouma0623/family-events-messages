 ...
 * URL優先順位（自治体/公式 > その他）
 */
export function mergeEvents(event1: EventNormalized, event2: EventNormalized): EventNormalized {
  // 基本はevent1をベースに、event2の方が良い場合は上書き
  const merged: EventNormalized = { ...event1 };
  ...
  // updatedAtを更新
  merged.updatedAt = getCurrentISO8601();
